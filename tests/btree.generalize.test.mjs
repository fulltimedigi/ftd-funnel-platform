 ...
 * degenerate tree appear? A collapse is SHOWN (a ✗), never fixed here.
 */
...
// A: ONE DOMINANT axis (type 8 values; budget & origin single-value) — expect type branches, others gated (gain 0)
...
function verify7(name, cat) {
  ...
  // SC1 axis rule re-run (from transcript counts)
  const statsAt = (poolRef) => { const t = oracle.transcript().filter((e) => e.transition_kind === "PROBE" && e.parent_pool_ref === poolRef); const byAxis = {}; for (const e of t) (byAxis[e.axis_id] ||= new Map()).set(e.option_ref, { exact: e.exact_count, eligible: e.eligible_count }); return Object.fromEntries(Object.entries(byAxis).map(([a, m]) => [a, [...m.values()]])); };
  for (const { node, axisId } of tree.internalChoices) assert.equal(chooseAxisByInfoGainV3(statsAt(node.pools.eligible_ref), { minExactRatio: MINR }), axisId, "SC1 rule re-run");
  // SC2 accumulation
